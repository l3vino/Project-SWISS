...
import { PerfPanel } from './ui/perf.js';
...
const live = { camera: null, view: null, info: null };
let perf = null;
...
async function start() {
  ...
  console.info('[swissgpu]', {
    gpu: info.gpu, adapter: info.adapter, features: info.features, blockCompression: info.bc,
...
function wireInterface(render, canvasEl) {
  ...
  const pressAction = (action) => {
    ...
    else if (action === 'perf') settings.set('showPerf', !settings.get('showPerf'));
  };

  perf = new PerfPanel({
    onPause: (on) => render.send('pause-streaming', { on }),
    onClose: () => settings.set('showPerf', false),
    context: () => ({ settings: settings.snapshot(), info: live.info }),
  });
  settings.on('showPerf', (v) => perf.visible(v));
  perf.visible(settings.get('showPerf'));
  ...
  addEventListener('keydown', (e) => {
    ...
    for (const action of ['menu', 'readout', 'perf']) {
