...
export class WalkController {
  ...
  step(dt, keys, pressed, camera, ground, p, solids = null) {
    ...
    /* ---- jump: on a press, and again the instant you land while held ---- */
    // The Space that switched into walking has to be let go once first, or a
    // double-tap on the ground would also jump.
    if (!(keys & KEY.UP)) this.jumpArmed = true;
    if (this.grounded && ((pressed & KEY.UP) || ((keys & KEY.UP) && this.jumpArmed))) this.#jump(feet, p);
    ...
    if (!this.grounded) {
      ...
      if (feet <= floor) {
        ...
        // Space still held: straight back up, in the same step the feet touch.
        if ((keys & KEY.UP) && this.jumpArmed) this.#jump(feet, p);
      }
    }
