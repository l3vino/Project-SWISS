 ...
 * of time, eased in and out. It turns to look along the path and down
 * towards where it is going, and over the descent turns back, so that it
 * lands looking exactly the way the camera looked when it took off: the
 * same compass heading and the same pitch. Any key or mouse movement hands
...
/* Fractions of the flight: turning onto the path, and turning back. */
const TURN_IN = 0.2;
const TURN_BACK = 0.75;
...
export class FlyTo {
  ...
  step(dt, camera, floorAt) {
    ...
    // Turn onto the path's heading at the start and look down towards the
    // destination while travelling; over the descent, turn back to the
    // angles the flight began with, arriving on them exactly.
    const turnIn = ease(Math.min(1, t / TURN_IN));
    const turnBack = t > TURN_BACK ? ease(Math.min(1, (t - TURN_BACK) / (1 - TURN_BACK))) : 0;
    ...
    const pathYaw = this.yaw0 + wrap(this.heading - this.yaw0) * turnIn;
    const pathPitch = this.pitch0 + (cruise - this.pitch0) * turnIn;
    let yaw = pathYaw + wrap(this.yaw0 - pathYaw) * turnBack;
    let pitch = pathPitch + (this.pitch0 - pathPitch) * turnBack;
    if (t >= 1) { yaw = this.yaw0; pitch = this.pitch0; }
