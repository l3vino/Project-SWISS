...
export class Frame {
  ...
  #tick(now) {
    ...
    // what the display actually showed; time spent inside render() alone is
    // not. Uncapped: `dt` is limited for the simulation's sake, not this.
    this.history[this.historyAt] = Math.max(this.interval, performance.now() - now);
