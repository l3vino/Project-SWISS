...
const PLAIN = new Set(['F1', 'F2', 'F3', 'F5', 'F6', 'F7', 'F10', 'Tab', 'Backspace', 'Space']);

/* Mouse movement goes to the render thread at most this often, milliseconds. */
const LOOK_INTERVAL_MS = 2;
...
export class InputBridge {
  ...
  constructor(canvas, endpoint, { onLockChange, onAction, bindings = DEFAULT_BINDINGS, captureKeys = true, doubleTapMs = 300 } = {}) {
    ...
    document.addEventListener('pointerlockchange', () => {
      ...
      if (!this.locked) { this.#setMask(0); this.#releaseKeyboard(); this.lookX = this.lookY = 0; }
    ...
    /* Mouse movement close to the mouse's own rate, not once per page frame.
     ...
     * `pointerrawupdate` fires for every report the mouse sends. Gaming mice
     * send one to eight thousand a second, though, and a message each is that
     * many copies and tasks on both threads for nothing: movement is summed
     * here and sent at most every LOOK_INTERVAL_MS, and whatever is left when
     * the mouse stops follows that long after. Still several per frame at
     * any frame rate a display shows. */
    this.lookX = 0;
    this.lookY = 0;
    this.lookSentAt = -Infinity;
    this.lookTimer = 0;
    const look = (e) => {
      if (!this.locked || !(e.movementX || e.movementY)) return;
      this.lookX += e.movementX;
      this.lookY += e.movementY;
      if (e.timeStamp - this.lookSentAt >= LOOK_INTERVAL_MS) this.#sendLook(e.timeStamp);
      else if (!this.lookTimer) this.lookTimer = setTimeout(() => this.#sendLook(performance.now()), LOOK_INTERVAL_MS);
    };
  ...
  #sendLook(now) {
    ...
    this.endpoint.send('look', { dx: this.lookX, dy: this.lookY });
