...
export class Search {
  ...
  #buildScopeSwitch() {
    // Each side says what it searches, on hover and to screen readers.
    const modes = [
      ['global', 'Global', 'Cities and towns worldwide'],
      ['national', 'National', 'Addresses, summits and places in the country on screen'],
    ];
    for (const [value, label, what] of modes) {
      const b = document.createElement('button');
      b.className = 'scope';
      b.type = 'button';
      b.textContent = label;
      b.title = what;
      b.setAttribute('aria-label', `${label}: ${what}`);
      b.dataset.scope = value;
      b.setAttribute('aria-pressed', String(value === this.scope));
      b.addEventListener('click', () => { this.#setScope(value); this.input.focus(); });
