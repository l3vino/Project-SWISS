...
export class Readout {
  ...
  update(stats) {
    ...
    if (t) {
      ...
      // Detail lowered to fit the memory budget shows here, in red.
      const lowered = t.detail != null && t.detail < 0.995;
      this.set('levels', t.drawn ? `z${t.minLevel}–${t.maxLevel} · ${(t.bytes / MB).toFixed(0)} MB` +
        (lowered ? ` · detail ${Math.round(t.detail * 100)}%` : '') : '—', lowered);
    }
    ...
    if (stats.work != null) {
      ...
      this.set('worst', `${stats.late} in 5 s`, stats.late > 2);
    }
    this.set('gpu', stats.gpuMs >= 0 ? `${stats.gpuMs.toFixed(2)} ms · 95th ${stats.gpu.frame95.toFixed(1)}` : 'not measured');
