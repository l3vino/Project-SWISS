/* open-meteo.js — populated places, worldwide.
 *
 * What the Global search mode asks. It indexes GeoNames, so it answers for
 * cities, towns and villages anywhere on earth: no key, CORS enabled, free for
 * non-commercial use with the data under CC BY 4.0.
 *
 * It is not unlimited. The free tier allows fewer than 10,000 calls a day,
 * 5,000 an hour and 600 a minute (open-meteo.com/en/terms). Search is
 * debounced to one request per pause in typing, which keeps one person far
 * below that; anything that ever queries in bulk has to respect it.
 *
  ...
  search: {
    ...
    map(place) {
      ...
      return {
        ...
        lon: place.longitude,
        lat: place.latitude,
        // Metres above sea level from a 90 m model: a first guess at where
        // the ground is, until terrain tiles say exactly.
