 ...
 *   offset 8   uint16  x2   which building of the tile it belongs to; the
 *                           facing of the wall this vertex starts, as a
 *                           diamond angle
 *
 ...
 *   mesh_quantize   once: every vertex into the box, with its building's number
 *   mesh_indices    once: drop hidden buildings' triangles, narrow to 16 bits
 *   mesh_features   once: each building's height, eaves, footprint and roof
 *   mesh_facets     once: give every wall triangle its exact facing
 *   mesh_narrow     once, when every vertex fits: 32-bit indices to 16
 *
 * What a building looks like is not decided here: its number indexes a
 * record the decode thread writes from the measurements (see
 * js/engine/features/appearance.js), which the shader reads.
 */
 ...
 *
 * @param kindsOff     u8 per feature: what kind of thing it is, or 0
 * @param hiddenKinds  bit k set: kind k is not drawn
 * Returns the number of indices written.
 */
EXPORT u32 mesh_indices(u32 idxOff, u32 count, u32 featOff, u32 featureCount, u32 kindsOff,
                        u32 hiddenKinds, u32 outOff, u32 narrow) {
  const u32 *idx = (const u32 *)PTR(idxOff);
  const u32 *feat = featOff ? (const u32 *)PTR(featOff) : 0;
  const u8 *kinds = kindsOff ? (const u8 *)PTR(kindsOff) : 0;
  u16 *out16 = (u16 *)PTR(outOff);
  u32 *out32 = (u32 *)PTR(outOff);
  u32 w = 0;
  for (u32 t = 0; t + 2 < count; t += 3) {
    if (feat && kinds && hiddenKinds) {
      u32 k = feat[idx[t]];
      if (k < featureCount && ((hiddenKinds >> (kinds[k] & 31u)) & 1u)) continue;
    }
    for (u32 j = 0; j < 3; j++) {
...
/* ---- what each building is like ------------------------------------------ */

/*
 * The style rules (js/engine/features/appearance.js) and the facade shader
 * want a few numbers per building that only its geometry knows: how tall it
 * is, where its walls stop and its roof begins (the eaves, whose overhang
 * shades the top of the wall), how much ground it covers and whether its
 * roof is pitched. Measured here over the triangles that are drawn, in the
 * tile's local frame, before mesh_facets adds any vertices.
 *
 * A roof face is one tilted less than 70° from level whose lowest corner is
 * more than a metre above the building's lowest point, which leaves out the
 * floor. The eaves are the lowest corner of any roof face; a building with no
 * such face has them at its top.
 *
 * Per feature, FEATURE_FLOATS floats: lowest, eaves, top (local metres), roof
 * plan area, pitched plan area (faces steeper than about 8°), and the plan
 * bounds: min x, min y, max x, max y.
 */

#define FEATURE_FLOATS 9
#define ROOF_MIN_NZ 0.34f        /* cos 70° */
#define PITCH_MAX_NZ 0.99f       /* cos 8° */
#define FLOOR_CLEARANCE 1.0f

EXPORT void mesh_features(u32 localOff, u32 vertexCount, u32 idxOff, u32 indexCount,
                          u32 featOff, u32 featureCount, u32 outOff) {
  const float *l = (const float *)PTR(localOff);
  const u32 *idx = (const u32 *)PTR(idxOff);
  const u32 *feat = featOff ? (const u32 *)PTR(featOff) : 0;
  float *out = (float *)PTR(outOff);
  ...
  for (u32 k = 0; k < features; k++) {
    float *o = out + k * FEATURE_FLOATS;
    o[0] = 3.0e38f; o[1] = 3.0e38f; o[2] = -3.0e38f; o[3] = 0.0f; o[4] = 0.0f;
    o[5] = 3.0e38f; o[6] = 3.0e38f; o[7] = -3.0e38f; o[8] = -3.0e38f;
  }

  /* Extent: lowest and highest points, and the plan bounds. */
  for (u32 t = 0; t + 2 < indexCount; t += 3) {
    for (u32 j = 0; j < 3; j++) {
      ...
      if (k >= features) k = 0;
      float *o = out + k * FEATURE_FLOATS;
      float x = l[i * 3], y = l[i * 3 + 1], z = l[i * 3 + 2];
      if (z < o[0]) o[0] = z;
      if (z > o[2]) o[2] = z;
      if (x < o[5]) o[5] = x;
      if (y < o[6]) o[6] = y;
  ...
  /* Roofs: where they start, how much ground they cover, how much is pitched. */
  for (u32 t = 0; t + 2 < indexCount; t += 3) {
    ...
    if (k >= features) k = 0;
    float *o = out + k * FEATURE_FLOATS;
    const float *pa = l + a * 3, *pb = l + b * 3, *pc = l + c * 3;
    float ux = pb[0] - pa[0], uy = pb[1] - pa[1], uz = pb[2] - pa[2];
    float vx = pc[0] - pa[0], vy = pc[1] - pa[1], vz = pc[2] - pa[2];
    float nx = uy * vz - uz * vy, ny = uz * vx - ux * vz, nz = ux * vy - uy * vx;
    float len = (float)m_sqrt((double)(nx * nx + ny * ny + nz * nz));
    if (len <= 0.0f) continue;
    float anz = nz < 0.0f ? -nz : nz;
    if (anz < ROOF_MIN_NZ * len) continue;                 /* a wall */
    float low = pa[2];
    if (pb[2] < low) low = pb[2];
    if (pc[2] < low) low = pc[2];
    if (low < o[0] + FLOOR_CLEARANCE) continue;            /* the floor */
    if (low < o[1]) o[1] = low;
    float plan = 0.5f * anz;                               /* |nz| is twice the plan area */
    o[3] += plan;
    if (anz < PITCH_MAX_NZ * len) o[4] += plan;
  }
