 ...
 * its vertices are quantised into, and where the frame sits on the imagery
 * grid so roofs can sample the aerial photograph.
 *
...
export const BUILDING_VERTEX_LAYOUT = {
  ...
  attributes: [
    ...
    { shaderLocation: 1, offset: 8, format: 'uint16x2' },    // kind << 12 | tint, wall facing
  ],
};

export class BuildingsPass {
  static async create(device, { format, timer, tilesets, imagery, frame }) {
    const module = await loadShader(device, 'buildings',
      ['common.wgsl', 'frame.wgsl', 'imagery.wgsl', 'buildings.wgsl']);
    ...
    return new BuildingsPass(device, pipeline, blockLayout, { timer, tilesets, imagery, frame });
  }

  constructor(device, pipeline, blockLayout, { timer, tilesets, imagery, frame }) {
    this.device = device;
  ...
  record(encoder, colorView, depthView) {
    ...
    if (this.draws.length) {
      ...
      for (let i = 0; i < this.draws.length; i++) {
        ...
        pass.setBindGroup(1, this.bind, [i * BLOCK_STRIDE]);
