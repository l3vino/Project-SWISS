...
export class Atmosphere {
  static async create(device, frame) {
    ...
    return new Atmosphere(device, frame, { transmittance, multiple, skyView, aerial, ambient });
  }

  constructor(device, frame, pipelines) {
    this.device = device;
    this.frame = frame;
  ...
  record(encoder) {
    ...
    if (off(height, m.height) || sunElevation !== m.sun) {
      this.#dispatch(encoder, 'skyView', [Math.ceil(192 / 8), Math.ceil(108 / 8), 1]);
      m.height = height;
    }
    if (off(ground, m.ground) || sunElevation !== m.sun) {
      this.#dispatch(encoder, 'ambient', [1, 1, 1]);
      m.ground = ground;
    }
    m.sun = sunElevation;
    this.#dispatch(encoder, 'aerial', [32 / 8, 32 / 8, 1]);
  }
}
