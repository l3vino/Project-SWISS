...
// What a mirror facing direction `r` shows: the sky's gradient above the
// horizon, matching the sky pass, and dim ground below it. For glass and,
// later, water.
fn skyReflection(r : vec3f, up : vec3f) -> vec3f {
  let zenith = vec3f(0.0066, 0.0220, 0.1193);
  let h = dot(r, up);
  let sky = mix(frame.horizon, zenith, pow(clamp(h, 0.0, 1.0), 0.55));
  let ground = mix(frame.horizon * 0.35, vec3f(0.035, 0.034, 0.03), smoothstep(0.0, -0.25, h));
  return select(sky, ground, h < 0.0);
}
