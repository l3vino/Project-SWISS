...
export class Tileset {
   ...
   */
  constructor({ device, pool, spec, maxError = 10, budgetMB = 256 }) {
    this.device = device;
    this.pool = pool;
    this.spec = spec;
  ...
  configure({ maxError, budgetMB, enabled }) {
  ...
  #visit(node) {
    ...
    if (!view.sphereVisible(vol.center, vol.radius)) return;
    ...
    if (node.external) {
      if (node.state !== READY) { this.#request(node, d); return; }
      for (const c of node.children) this.#visit(c);
      return;
    }

    const refine = node.geometricError * this.errorFactor > d;
    if (!refine || !node.children.length) { this.#show(node, d); return; }
    ...
    // REPLACE: children take over once every one in view can stand in.
    let ready = true;
    for (const c of node.children) {
      c.lastUsed = this.frame;
      const dc = this.#distance(c);
      const inView = view.sphereVisible(c.volume.center, c.volume.radius) && dc <= view.horizon;
      if (!c.settled) this.#request(c, inView ? dc : 1e6 + dc * 4);
      if (inView && !this.#canStandIn(c, 0)) ready = false;
    }
    if (!ready && node.url && node.state === READY) { this.#show(node, d); return; }
    for (const c of node.children) this.#visit(c);
  }
  ...
  #canStandIn(node, depth) {
    ...
    for (const c of node.children) {
      c.lastUsed = this.frame;
      if (!this.view.sphereVisible(c.volume.center, c.volume.radius)) continue;
      if (!c.settled) this.#request(c, this.#distance(c));
      if (!this.#canStandIn(c, depth + 1)) return false;
    }
    return true;
  }

  /** Draws a tile's content if it is here, asks for it if not. */
  #show(node, d) {
  ...
  async #loadContent(node) {
    ...
    this.inFlight++;
    try {
      const result = await this.pool.run('feature-tile', {
        url: node.url,
  ...
  #upload(node, r) {
    ...
    const bytes = r.vertices.byteLength + r.indices.byteLength;
    node.gpu = {
      vbuf, ibuf, bytes,
