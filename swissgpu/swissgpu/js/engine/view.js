...
export class View {
  constructor() {
    ...
    this.horizon = Infinity;              // metres beyond which nothing can be seen
    this.version = 0;
  ...
  update(camera, { viewHeight, fovY, aspect, near }, ground = 0) {
    ...
    this.horizon = Math.sqrt(2 * WGS84_A * Math.max(1, camera.height - ground)) +
  ...
  setFocus(lon, lat, radius) {
    ...
    this.focus = {
      lon, lat, radius,
      center: geodeticToEcef(lon, lat, 0),
      up: enuBasis(lon, lat).up,
      mPerDegLat: meridian * DEG,
      mPerDegLon: primeVertical * Math.cos(lat * DEG) * DEG,
    };
    this.version++;
  }
