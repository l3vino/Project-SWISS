...
function wireInterface(render, canvasEl) {
  ...
  new Search(registry, {
    view: () => live.view,
    onPick(place) {
      // Standing on the ground at the place, in walking mode. The render
      // thread settles the eye onto the terrain as it loads; where there is
      // no terrain it arrives flying instead and says why. A smooth flight
      // there, rather than a jump, comes with step 5.
      render.send('spawn', {
