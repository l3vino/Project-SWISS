 ...
 * vertex, drop hidden buildings' triangles, give every wall its facing. Tiles near the camera also get a
 * collision grid over those same triangles (wasm/src/solid.c), so what you
 * bump into is exactly what is drawn.
 *
...
export async function decodeFeatureTile(core, buffer, { transform, upAxis = 'Y', frame, attributes = {}, solid = false }) {
  ...
  // layer really holds is how its `kinds` table gets tuned.
  ...
  return {
    ...
    solid: grid,
    $transfer: grid ? [vertices, indexBuffer, grid.cellStart, grid.cellTris] : [vertices, indexBuffer],
  };
}
...
function classify(batchTable, batchBinary, count, attributes) {
  ...
  for (let k = 0; k < count; k++) {
    const kind = KIND[kinds[types?.[k]] ?? attributes.defaultKind] ?? KIND.building;
