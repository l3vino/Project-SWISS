 ...
 * primitive on the earth, classifies each building from its attributes, and
 * hands the heavy per-vertex work to the C core (wasm/src/mesh.c): transform
 * into the tile's local east-north-up frame, quantise to twelve bytes a
 * vertex, drop hidden buildings' triangles.
 *
...
/* What a building is, as far as drawing it goes. The shader reads the same
 * numbers from the top four bits of each vertex's info. */
export const KIND = { building: 0, plain: 1, glass: 2, canopy: 3, hidden: 15 };
const HIDDEN_KINDS = 1 << KIND.hidden;
 ...
 * @param attributes the adapter's description of its batch table (see adapters)
 */
export async function decodeFeatureTile(core, buffer, { transform, upAxis = 'Y', frame, attributes = {} }) {
  ...
  const info = classify(batchTable, batchBinary, featureCount, attributes);
  ...
  const infoOff = core.write(new Uint8Array(info.buffer));
  const lowestOff = core.alloc(featureCount * 4);
  const vertexOff = core.alloc(vertexCount * BUILDING_VERTEX_BYTES);
  core.exports.mesh_quantize(localOff, vertexCount, boundsOff, featOff, featureCount, infoOff, lowestOff, vertexOff);

  const narrow = vertexCount <= 65536;
  const idxOff = core.write(new Uint8Array(indices.buffer));
  const idxOutOff = core.alloc(indexCount * (narrow ? 2 : 4));
  const kept = core.exports.mesh_indices(idxOff, indexCount, featOff, featureCount, infoOff,
    HIDDEN_KINDS, idxOutOff, narrow ? 1 : 0);
  core.sync();

  const bounds = Array.from(core.f32.subarray(boundsOff / 4, boundsOff / 4 + 6));
  const vertices = core.buffer.slice(vertexOff, vertexOff + vertexCount * BUILDING_VERTEX_BYTES);
  ...
  return {
    vertices, indices: indexBuffer, vertexCount, indexCount: kept, indexFormat: narrow ? 'uint16' : 'uint32',
    ...
    encoding, features: featureCount,
    $transfer: [vertices, indexBuffer],
  };
}

/**
 * Per building, sixteen bits for the shader: its kind in the top four, a
 * stable tint in the rest. The tint comes from the building's own id where
 * the source has one, so the same building keeps its colour in every tile
 * and at every level of detail.
 */
function classify(batchTable, batchBinary, count, attributes) {
  const info = new Uint16Array(count);
  const types = attributes.type ? batchColumn(batchTable, batchBinary, attributes.type, count) : null;
  const ids = attributes.id ? batchColumn(batchTable, batchBinary, attributes.id, count) : null;
  const kinds = attributes.kinds || {};
  for (let k = 0; k < count; k++) {
    const kind = KIND[kinds[types?.[k]]] ?? KIND.building;
    const tint = hash(ids?.[k] ?? k) & 0x0fff;
    info[k] = (kind << 12) | tint;
  }
  return info;
}
