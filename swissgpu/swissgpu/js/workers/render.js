...
const profiler = new Profiler(['physics', 'uploads', 'view', 'terrain', 'buildings', 'ground', 'imagery', 'draw', 'submit']);
...
rpc.on('init', async ({ canvas, wasmModule, settings, size, camera: startCamera }) => {
  ...
  const atmosphere = await Atmosphere.create(device, frameUniforms);
...
function step(dt) {
  ...
  // Tiles that arrived since the last frame, a slice's worth.
  uploads.run();
