 ...
 */
rpc.on('feature-tile', async ({ url, transform, upAxis, frame, attributes }) => {
  const response = await fetch(url);
  if (response.status === 404 || response.status === 403 || response.status === 204) return { missing: true };
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  return decodeFeatureTile(core, await response.arrayBuffer(), { transform, upAxis, frame, attributes });
});
