...
export class Readout {
  ...
  update(stats) {
    ...
    if (m) {
      ...
      this.set('speed', m.mode === 'walk'
        ? `${m.speed.toFixed(1)} m/s`
        : `${Math.round(m.speed)} m/s${m.boosted ? ' ×3' : ''}`, m.boosted);
      this.set('ground', m.aboveGround == null ? '—'
        : m.aboveGround < 100 ? `${m.aboveGround.toFixed(2)} m`
        : `${Math.round(m.aboveGround).toLocaleString('en')} m`);
    }
