...
export async function run(page, { shot } = {}) {
  ...
  const wrong = [];
  for (const t of p.buildingTiles) {
    const want = expectedTriangles(t.url);
    if (!want || want.drawn !== t.triangles) wrong.push(`${t.url.split('/v1/')[1]}: ${t.triangles} (want ${want?.drawn})`);
  }
  check('every tile draws its houses, less the underground one', p.buildingTiles.length > 0 && !wrong.length,
    wrong.length ? wrong.slice(0, 4).join('; ') : `${p.buildingTiles.length} tiles, triangle counts exact`);
  const detailed = p.buildingTiles.filter((t) => expectedTriangles(t.url)?.detailed);
  check('up close, detailed tiles have replaced the coarse ones', detailed.length >= 4,
    `${detailed.length} detailed and ${p.buildingTiles.length - detailed.length} coarse tiles drawn`);
  ...
  p = await until((q) => q.buildings.drawn === 0, 30000);
  const off = p.buildings.drawn;
  await send('setting', { id: 'buildings', value: true });
  p = await until((q) => q.buildings.drawn > 0, 60000);
  check('Layers → Buildings turns them off and on', off === 0 && p.buildings.drawn > 0,
    `off: ${off} tiles drawn, back on: ${p.buildings.drawn}`);
