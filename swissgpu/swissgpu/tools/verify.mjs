...
import { BUILDINGS_PATH, serveBuildings, STRUCTURES_PATH, serveStructures } from './mock-buildings.mjs';
...
const counts = {
  ...
  buildingTilesets: 0, buildingTiles: { plain: 0, meshopt: 0, draco: 0 }, structures: 0,
  other: 0,
};
await context.route(/^https:\/\//, async (route) => {
  ...
  if (url.host === '3d.geo.admin.ch' && url.pathname.startsWith(STRUCTURES_PATH)) {
    const answer = serveStructures(url.pathname);
    if (!answer) return route.fulfill({ status: 403, headers: cors, body: '' });
    counts.structures++;
    return route.fulfill({ status: answer.status, headers: { ...cors, 'content-type': answer.type }, body: answer.body });
  }
