...
/**
 * One house of the town, as the collision checks need it: centre, whether it
 * is a tower or the hidden underground one of its tile, and its heights
 * (metres above the ellipsoid): base, eaves, ridge. Half-extents are
 * HOUSE_HALF east and north.
 */
export function houseInfo(i, j) {
  const { cols } = townGrid();
  const { lon, lat } = houseCentre(i, j);
  const id = j * cols + i;
  let hidden = false;
  for (let q = 0; q < 4; q++) {
    for (let c = 0; c < 4; c++) {
      const r = quarter(quarter(AREA, q), c);
      if (lon < r.west || lon >= r.east || lat < r.south || lat >= r.north) continue;
      hidden = housesIn(r).findIndex((h) => h.id === id) === HIDDEN_INDEX;
    }
  }
  const tower = id % 11 === 5;
  const { east, north } = localScale({ lat });
  const w = HOUSE_HALF.east, d = HOUSE_HALF.north;
  const base = Math.min(...[[-w, -d], [w, -d], [w, d], [-w, d]]
    .map(([x, y]) => height(lon + x / east, lat + y / north))) - 0.4;
  const eaves = base + (tower ? 28 : 6.5);
  return { lon, lat, id, tower, hidden, base, eaves, ridge: tower ? eaves : eaves + 4 };
}

/* ---- structures: a bridge ------------------------------------------------ */
/* swissTLM3D's 3D objects stand in as one tile holding one bridge: a level
 * deck leaving the hillside east of the stand-in terrain's ridge at 8.8° E
 * and running east over the falling slope on two piers, the way a viaduct
 * leaves a hillside. Its first metres are set just into the slope, as an
 * abutment is, so a walker steps onto it from the ground and, walking on,
 * stays at deck height while the ground drops away. Every part is a closed
 * box, as in the real model, so a line straight up from under the deck passes
 * into and out of it (outside), where one from inside a house passes through
 * its roof once (inside). */

export const STRUCTURES_PATH = '/ch.swisstopo.swisstlm3d.3d/v1/';
const BRIDGE_START = { lon: 8.8 + 20 / (111320 * Math.cos((46.1515 * Math.PI) / 180)), lat: 46.1515 };
export const BRIDGE = {
  start: BRIDGE_START,
  length: 120,                      // metres east
  halfWidth: 3.5,
  thickness: 1.2,
  top: height(BRIDGE_START.lon, BRIDGE_START.lat) - 0.3,
  piers: [40, 80],                  // metres along, each 2 m by 4 m
};
const BRIDGE_TRIANGLES = 12 * (1 + BRIDGE.piers.length);

/* A closed box as twelve triangles, corners in local metres. */
...
function bridgeTriangles() {
  const B = BRIDGE, scale = localScale(B.start);
  // Heights relative to the deck top; the pier feet go a metre into the slope.
  const tris = box(0, B.length, -B.halfWidth, B.halfWidth, -B.thickness, 0);
  for (const x of B.piers) {
    const ground = height(B.start.lon + x / scale.east, B.start.lat) - B.top;
    tris.push(...box(x - 1, x + 1, -2, 2, ground - 1, -B.thickness));
  }
  return tris;
}

function structuresTileset() {
  const B = BRIDGE, scale = localScale(B.start);
  const rect = {
    west: B.start.lon - 5 / scale.east, east: B.start.lon + (B.length + 5) / scale.east,
    south: B.start.lat - 10 / scale.north, north: B.start.lat + 10 / scale.north,
  };
  return {
    asset: { version: '1.0', gltfUpAxis: 'Y' },
    geometricError: 500,
    root: {
      boundingVolume: { region: region(rect, B.top - 150, B.top + 5) },
      geometricError: 200,
      refine: 'ADD',
      children: [{
        boundingVolume: { region: region(rect, B.top - 150, B.top + 5) },
        geometricError: 0,
        content: { uri: 'bridge.b3dm' },
      }],
    },
  };
}
...
function bridgeContent() {
  const B = BRIDGE;
  const origin = geodeticToEcef(B.start.lon, B.start.lat, B.top);
  const { east, north, up } = enuBasis(B.start.lon, B.start.lat);
  const rtc = Array.from(origin);
  const positions = [], batch = [], indices = [];
  for (const [x, y, z] of bridgeTriangles()) {
    const X = east[0] * x + north[0] * y + up[0] * z;
    const Y = east[1] * x + north[1] * y + up[1] * z;
    const Z = east[2] * x + north[2] * y + up[2] * z;
    indices.push(positions.length / 3);
    positions.push(X, Z, -Y);
    batch.push(0);
  }
  const mesh = { positions: new Float32Array(positions), batch: new Float32Array(batch), indices: new Uint32Array(indices) };
  return b3dm(gltfPlain(mesh), { BATCH_LENGTH: 1, RTC_CENTER: rtc }, { OBJEKTART: ['Bruecke'] });
}

/** Triangles the bridge tile draws, for the decoder check. */
export function expectedStructureTriangles(url) {
  const path = new URL(url, 'https://3d.geo.admin.ch').pathname;
  return path === `${STRUCTURES_PATH}bridge.b3dm` ? BRIDGE_TRIANGLES : null;
}

/** Answers a request path under STRUCTURES_PATH. */
export function serveStructures(path) {
  const rel = path.slice(STRUCTURES_PATH.length);
  if (rel === 'tileset.json') return { status: 200, type: 'application/json', body: JSON.stringify(structuresTileset()) };
  if (rel === 'bridge.b3dm') return { status: 200, type: 'application/octet-stream', encoding: 'plain', body: bridgeContent() };
  return null;
}
