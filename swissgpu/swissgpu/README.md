...
45 cm. Switching to walking in mid-air drops you with real gravity and air
drag, so from 6,000 m the fall takes about two minutes. Ground steeper than
45° cannot be climbed and slides you off. All of these, and your height, are
in Settings → Physics.
...
are all requested at once, so this takes about a second. Outside Switzerland
there is no terrain yet, so you arrive flying 150 m above the place instead,
...
- **Native core.** The C under `wasm/src/` is compiled to a 19 KB wasm module,
...
- **Buildings.** All of swissBUILDINGS3D, streamed as 3D Tiles: nested
  ...
  within a few centimetres across a tile; walls get plaster colours, windows
  that fade to their average with distance instead of shimmering, and
  darkening where they meet the ground. Normals come from the screen-space
  derivatives of position, exact for flat faces and free in memory.
  Settings → Layers turns them off and sets their detail and memory. You can
  still walk through them for now: collision is a later step.
...
- **Physics.** Flying and walking are separate controllers stepping at 120 Hz on
  ...
  ones behind you that are not drawn; a lookup costs about 0.05 µs.
...
## Layout
    ...
    js/engine/features/          the 3D Tiles streamer
...
slides off a slope it cannot climb, flies up and falls back down, and checks
each against the physics, measured on the simulation's own clock.
...
| Roads, railways, water, trees, names | 3D Tiles, `3d.geo.admin.ch`, declared for the steps that draw them |
