...
opens the settings, **F3** shows or hides the readout. Every key can be
changed in Settings → Controls → Keys: click a key, press the new one.
...
requested when the flight starts, so they are there when it lands. Any key or
mouse movement takes over where you are. If the spot is inside a building,
...
- **Frame pacing.** Every GPU upload of arriving tiles goes through one queue
  that spends at most 2 ms of a frame (`js/engine/uploads.js`), so a burst of
  tiles spreads over a few frames instead of making one late. The mouse is
  read through `pointerrawupdate`, every report the mouse sends rather than one
  batch per page frame, and without the operating system's acceleration where
  the platform allows. A profiler (`js/engine/profiler.js`) splits each frame
  into physics, uploads, streaming and drawing; the readout shows the real work
  per frame, its 99th percentile, and how many frames came late.
...
## Layout
    ...
    js/ui/                       boot, readout, search, the settings menu
...
and prints where each frame's time went, section by section, with the worst
frame of each; it checks that uploads stayed inside their slice.
