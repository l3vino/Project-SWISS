...
export async function run(page, { shot } = {}) {
   ...
   * looking exactly the way the camera looked when it took off. The start
   * faces away from the destination and down, so the flight has to turn
   * round and back again. */
  await send('camera', { state: { ...(await probe()).camera, yaw: -2.2, pitch: -0.45 } });
  ...
  const turn = Math.abs(Math.atan2(Math.sin(q.camera.yaw - from.camera.yaw), Math.cos(q.camera.yaw - from.camera.yaw)));
  check('it lands looking the way it took off', turn < 1e-3 && Math.abs(q.camera.pitch - from.camera.pitch) < 1e-3,
    `heading ${from.camera.yaw.toFixed(3)} → ${q.camera.yaw.toFixed(3)}, pitch ${from.camera.pitch.toFixed(3)} → ${q.camera.pitch.toFixed(3)}`);
  await snap('landed');
  return results;
}
