 ...
 * the tile drawn under the eye is exactly as fine as the detail distance asks,
...
export async function run(page, { shot, counts } = {}) {
   ...
   * as fine as the detail distance asks: fine enough, and no finer. The
   * finest level (18) is kept out to that distance and each coarser one
   * twice as far, so a tile of level z splits while its error times
   * errorFactor exceeds its distance. */
  ...
  p = await settle((q) => q.camera.pitch < -1.2 && q.leaf);
  {
    ...
    const detailKm = (p.errorFactor * geometricError(18)) / 1000;
    const fineEnough = geometricError(z) * p.errorFactor <= d;
    const noFiner = geometricError(z - 1) * p.errorFactor > d;
    check('detail under the eye matches the detail distance', fineEnough && noFiner && z >= 10,
      `level ${z} under the eye at ${Math.round(d)} m, with full detail kept out to ${detailKm.toFixed(2)} km`);
  ...
  // Tiles in use this frame are never dropped; when those alone need more
  // than the budget, distant detail is lowered until they fit. The check is
  // that memory goes down, and the view (finest ground underfoot) survives.
  check('the memory budget is kept', q.terrain.ready < before.ready && q.terrain.bytes < before.bytes &&
    ...
    `${q.terrain.drawn} drawn, level ${q.leaf?.z} underfoot, detail ${Math.round(q.terrain.detail * 100)}%`);
