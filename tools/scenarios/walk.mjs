...
export async function run(page) {
  ...
  /* 5b. Space held down keeps jumping, straight back up the instant the
   * feet touch, and stops once it is let go. A jump is 0.61 s in the air, so
   * 2.5 s held is five take-offs, give or take a frame at either end. */
  ...
  check('holding Space keeps jumping, and stops when released',
    jumped >= 4 && jumped <= 5 && settled.jumps === p.jumps && Math.abs(p.lastJump - 0.45) < 0.01,
  ...
  if (slope > 4) {
    ...
    check('slope checks', slope != null, slope != null ? `skipped: ground here is only ${slope.toFixed(1)}°`
      : `not walking any more: ${p.motion.mode}, ${p.motion.state}`);
  }
