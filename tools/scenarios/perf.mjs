...
export async function run(page) {
  ...
  console.log(`  ... work ${fmt(p.work)} ms mean, ${fmt(p.work95)} 95th, ${fmt(p.work99)} ms 99th, ${p.late} late of ${p.frames}; ${split}`);
  const p95 = Object.entries(p.p95).map(([k, v]) => `${k} ${fmt(v)}`).join(', ');
  console.log(`  ... 95th percentile per section: ${p95}`);
  ...
  check('the upload slice keeps to its budget', flying.uploads.ms < flying.uploads.slice + 25,
